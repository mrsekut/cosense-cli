...
export async function pageCreate(parsed: ParsedArgs, format: Format): Promise<void> {
  const title = parsed.commands[2];
  if (!title) {
    output(error("MISSING_ARGUMENT", "Usage: cosense page create <title> --body <text> | --body-stdin"), format);
    return;
  }
