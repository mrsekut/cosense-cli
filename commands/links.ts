...
export async function linksCommand(parsed: ParsedArgs, format: Format): Promise<void> {
  const title = parsed.commands[1];
  if (!title) {
    output(error("MISSING_ARGUMENT", "Usage: cosense links <title> [--depth 1|2]"), format);
    return;
  }
  ...
  const oneHopLinks = page.relatedPages?.links1hop?.map((p: { title: string }) => p.title) ?? [];
  const twoHopLinks =
    depth >= 2
